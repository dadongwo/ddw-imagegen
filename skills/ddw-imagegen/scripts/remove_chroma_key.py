...
def _dependency_hint(package: str) -> str:
    return (
        "Use Codex's bundled Pillow-capable runtime to run this script directly. "
        "Windows (cmd): "
        "`\"%USERPROFILE%\\.cache\\codex-runtimes\\codex-primary-runtime\\dependencies\\python"
        "\\python.exe\" \"path\\to\\remove_chroma_key.py\" --check`. "
        "Unix: "
        "`\"$HOME/.cache/codex-runtimes/codex-primary-runtime/dependencies/python/bin/python\" "
        "/path/to/remove_chroma_key.py --check`. "
        f"If that runtime is unavailable, install {package} into the selected Python environment "
        f"with `uv pip install {package}`."
    )
...
def _validate_args(args: argparse.Namespace) -> None:
    if not args.input or not args.out:
        _die("--input and --out are required unless --check is used.")
...
def _check_runtime() -> None:
    ...
    print(f"Pillow runtime ready: Python {sys.version_info.major}.{sys.version_info.minor}, Pillow {version}")
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--check",
        action="store_true",
        help="Verify that the current Python runtime can import Pillow, then exit.",
    )
